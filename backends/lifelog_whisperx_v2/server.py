...
delle parole, ne sposta il 18-27% di oltre 1s e calcola le impronte vocali sulle
etichette di pyannote. Il v2:
  - Whisper large-v3 NON batched (faster-whisper diretto, fallback di temperatura,
    parola per parola con probabilità);
  - tempi delle parole con ctc-forced-aligner (MMS), durata massima 1.2s per parola;
  - pyannote community-1 come nel v1, esposto grezzo con i tratti sovrapposti;
  - impronte ResNet293 per FRASE e su FINESTRE scorrevoli, solo sull'audio delle
    parole e senza i tratti sovrapposti.
Nessuna identità: il backend non conosce la voce dell'utente né di altre persone.
La logica (chi è chi, raggruppamento, etichette) sta in Lifelog.

La risposta contiene tutti i campi del v1 (costruiti con lo stesso _to_contract del v1
ma sulle parole nuove) più il blocco `v2` descritto nel contratto.

Blackwell fixes (come v1):
...
LOG_FILE = r"C:\Users\Roberto\aria\logs\lifelog_whisperx_v2.log"
...
V2_VERSION = "lifelog_whisperx_v2/1"
WORD_MAX_S = 1.2        # ctc allunga l'ultima parola sul silenzio seguente (laboratorio 2026-09-25)
WORD_PAD_MS = 50        # margine attorno a ogni parola per le impronte
...
def _load_models():
    global _model, _align_model, _align_meta, _diarize_model, _voiceprint_model, _ctc_model, _ctc_tok
    import faster_whisper

    logger.info("Loading faster-whisper %s (non batched) on %s (%s) from %s ...",
                MODEL_SIZE, DEVICE, COMPUTE_TYPE, WHISPER_MODEL_PATH)
    t0 = time.time()
    _model = faster_whisper.WhisperModel(WHISPER_MODEL_PATH, device=DEVICE, compute_type=COMPUTE_TYPE)
    logger.info("ASR model loaded in %.1fs", time.time() - t0)

    # wav2vec2 del v1 non serve: i tempi vengono da ctc-forced-aligner
    _align_model = _align_meta = None
    t0 = time.time()
    from ctc_forced_aligner import load_alignment_model
    _ctc_model, _ctc_tok = load_alignment_model(DEVICE, dtype=torch.float16)
    logger.info("CTC aligner loaded in %.1fs", time.time() - t0)
    ...
    logger.info("Tutti i modelli v2 caricati (avvisi innocui come nel v1: torchcodec, TF32, checkpoint).")


def _unload_models():
    global _model, _align_model, _align_meta, _diarize_model, _voiceprint_model, _ctc_model, _ctc_tok
    _model = _align_model = _align_meta = _diarize_model = _voiceprint_model = None
    _ctc_model = _ctc_tok = None
    gc.collect()
...
def _asr_nonbatched(audio_np: np.ndarray, language: str) -> list[dict]:
    ...
    for k, s in enumerate(segs):
        out.append({
            "id": k, "start": s.start, "end": s.end, "text": s.text,
            "avg_logprob": s.avg_logprob, "no_speech_prob": s.no_speech_prob,
            "compression_ratio": s.compression_ratio, "temperature": s.temperature,
            "wwords": [{"start": w.start, "end": w.end, "word": w.word, "probability": w.probability}
                       for w in (s.words or [])],
        })
    return out


def _whisper_props(seg: dict) -> list[dict]:
    """Per ogni token di seg['text'].split(): probabilità minima e tempi Whisper delle
    parole Whisper che lo coprono (la tokenizzazione di Whisper spezza «l'alcol» in due)."""
    text = seg["text"]
    spans, pos = [], 0
    for w in seg["wwords"]:
        i = text.find(w["word"], pos)
        if i < 0:
            return [{} for _ in text.split()]
        spans.append((i, i + len(w["word"]), w)); pos = i + len(w["word"])
    out = []
    for m in re.finditer(r"\S+", text):
        cov = [w for a, b, w in spans if a < m.end() and b > m.start()]
        out.append({"prob": min(w["probability"] for w in cov),
                    "w_start": cov[0]["start"], "w_end": cov[-1]["end"]} if cov else {})
    return out
...
def _ctc_words(audio_np: np.ndarray, segs: list[dict]) -> tuple[list[dict], bool]:
    ...
    for i, ((seg_id, tok), p) in enumerate(zip(tokens, props)):
        if ctc:
            ...
            st, en, align = p.get("w_start"), p.get("w_end"), None
        if st is None:
            continue
        en = min(en, st + WORD_MAX_S)
        words.append({"i": len(words), "seg": seg_id, "word": tok,
                      "start": st, "end": en, "score": align,
                      "w_start": p.get("w_start"), "w_end": p.get("w_end"), "prob": p.get("prob")})
    return words, fallback


def _iso3(lang: str) -> str:
    return {"it": "ita", "en": "eng", "fr": "fra", "de": "deu", "es": "spa"}.get(lang, "ita")


def _overlap_frames(diarize_df) -> set[int]:
    """Frame da 10ms in cui pyannote sente >=2 parlanti."""
    act: dict[int, set] = {}
    for r in diarize_df.itertuples():
        for t in range(int(r.start * 100), int(r.end * 100)):
            act.setdefault(t, set()).add(r.speaker)
    return {t for t, s in act.items() if len(s) >= 2}


def _frames_to_crop(waveform: torch.Tensor, frames: list[int]) -> torch.Tensor | None:
    if not frames:
        return None
    ivs = []
    for t in frames:
        if ivs and ivs[-1][1] == t:
            ...
            ivs.append([t, t + 1])
    crops = [waveform[:, a * 160:b * 160] for a, b in ivs]
...
def _v2_embeddings(waveform: torch.Tensor, words: list[dict], over: set[int],
    ...
    for k, w in enumerate(words):
        by_seg.setdefault(w["seg"], []).append(k)
    for seg_id, ks in by_seg.items():
        allf = set().union(*(wframes[k] for k in ks))
        fr = sorted(allf - over)
        c = _frames_to_crop(waveform, fr)
    ...
    if words:
        ...
        while a < end:
            b = a + WIN_SIZE_MS
            win = set(range(a // 10, b // 10))
            fr = sorted(win & speech)
            items.append(("windows", {"start_ms": a, "end_ms": b, "speech_ms": len(fr) * 10,
                                      "overlap_ms": len(win & over) * 10}))
            crops.append(_frames_to_crop(waveform, fr) if fr else None)
            a += WIN_STEP_MS

    if embed_words:
        for k, w in enumerate(words):
            fr = sorted(wframes[k] - over)
            items.append(("words", {"i": w["i"], "speech_ms": len(fr) * 10}))
    ...
    for i, (kind, meta) in enumerate(items):
        meta["v"] = _f16b64(vec[i]) if vec.get(i) is not None else None
        (out["windows"]["items"] if kind == "windows" else out[kind]).append(meta)
    return out
...
app = FastAPI(title="Lifelog WhisperX v2", version="v2.1", lifespan=lifespan)
...
def health():
    ...
    return {
        "status": "ok",
        "model":  f"whisperx-{MODEL_SIZE}-v2", "version": V2_VERSION,
        "device": DEVICE,
        "vram_gb": vram,
...
def transcribe(req: TranscribeRequest):
    ...
    try:
        ...
        wx_result = {"segments": [], "word_segments": []}
        for s in nb_segs:
            ws = [{"word": w["word"], "start": w["start"], "end": w["end"], "score": w["score"]}
                  for w in by_seg.get(s["id"], [])]
            if not ws:
                continue
            wx_result["segments"].append({
                "start": ws[0]["start"], "end": ws[-1]["end"], "text": s["text"], "words": ws,
                "avg_logprob": s["avg_logprob"], "no_speech_prob": s["no_speech_prob"],
                "compression_ratio": s["compression_ratio"]})
            wx_result["word_segments"].extend(ws)
        ...
        for t in sorted(over):
            if ov_iv and ov_iv[-1][1] == t * 10:
                ov_iv[-1][1] = t * 10 + 10
            else:
                ov_iv.append([t * 10, t * 10 + 10])
        output["v2"] = {
            "version": V2_VERSION,
            "timing_s": timing,
            "asr": {
            ...
            "words": [{"i": w["i"], "seg": w["seg"], "word": w["word"],
                       ...
                       "w_start_ms": int(w["w_start"] * 1000) if w["w_start"] is not None else None,
                       "w_end_ms": int(w["w_end"] * 1000) if w["w_end"] is not None else None,
                       "prob": round(w["prob"], 4) if w["prob"] is not None else None,
                       "align": round(w["score"], 4) if w["score"] is not None else None,
                       "overlap": any(t in over for t in range(int(w["start"] * 100), int(w["end"] * 100)))}
                      for w in words],
            "diarization": {
                "model": "pyannote/speaker-diarization-community-1",
                "intervals": [{"start_ms": int(r.start * 1000), "end_ms": int(r.end * 1000), "label": r.speaker}
                              for r in diarize_segs.itertuples()],
                "overlaps": [{"start_ms": a, "end_ms": b} for a, b in ov_iv],
            },
            "embeddings": emb_block,
        }
        logger.info("v2 block: %d parole, %d frasi, %d finestre, embed %.1fs",
                    len(words), len(emb_block["phrases"]), len(emb_block["windows"]["items"]), timing["embed"])
