    ...
    def load_by_hash(
        ...
        for i, ref in enumerate(references):
            if use_cache == "off" or audio_hashes[i] not in self.ref_by_hash:
                # If the references are not already loaded, encode them
                if hasattr(ref, 'tokens') and ref.tokens:
                    import io, numpy as np
                    tokens = np.load(io.BytesIO(ref.tokens))
                    # Move exactly as encode_reference would return: [10, N]
                    tokens_tensor = torch.tensor(tokens, device=self.decoder_model.device)
                    if tokens_tensor.ndim == 3:
                        tokens_tensor = tokens_tensor[0]
                    
                    logger.info(f"Loaded tokens with shape: {tokens_tensor.shape}")
                    prompt_tokens.append(tokens_tensor)
                else:
