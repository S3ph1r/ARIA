...
### 🔧 FASE AS-6: Backend TTS Fish S1-mini — ✅ COMPLETATO

**🎯 Obiettivo**: Integrazione di Fish S1-mini, ottimizzazione prosodia italiana e workflow end-to-end.

**✅ Implementazione Finalizzata**:
- [x] **Fish S1-mini integration**: Backend Python che dialoga con API native Windows.
- [x] **Catalyst Prompting**: Implementazione della "Punteggiatura Drammatica" (`...`) per il ritmo.
- [x] **Official Markers**: Uso esclusivo di English Markers ufficiali (es. `(serious)`) per evitare allucinazioni fonetiche.
- [x] **Accent Fidelity**: Allineamento obbligatorio tra `voice_ref_text` e `narratore.wav` per accento italiano nativo.
- [x] **Title Pacing**: Iniezione di segue silenziosi (`... .`) dopo i titoli per enfasi narrativa.
- [x] **Performance**: RTF ~1:5 su Blackwell/RTX con torch.compile.
...
### 🔧 FASE AS-10: ARIA Windows Node Controller — ✅ IN CORSO
**🎯 Obiettivo**: Gestione integrata Tray App + Orchestratore.

**🔧 Status & Ottimizzazioni**:
- [x] **Orchestration**: Spostata la logica di bridge Redis su Windows per minore latenza.
- [x] **Model Switching**: VRAMManager ottimizzato per scarico immediato tra Fish e MusicGen.
- [x] **Semaforo**: Integrazione completa con la Gaming Mode per priorità assoluta all'utente.
- [ ] GUI definitiva in CustomTkinter.
