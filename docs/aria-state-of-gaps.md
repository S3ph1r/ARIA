...
**Verifica ancora da fare dopo il deploy:** confermare sul vivo (log + `nvidia-smi`/Task Manager) che uno swap qwen3→Flux2 ora produca le righe di conferma attese, che la finestra Console si chiuda davvero, e che il pid-file di qwen3-14b-q4km/altri backend venga popolato dalla scoperta via titolo anche senza self-reporting esplicito.
