...
**Stato:** in-progress (fix definitivo scritto il 2026-09-24 su LXC 190 — spawn via conhost.exe esplicito, elimina la delega a Windows Terminal alla radice — ancora da deployare su PC 139 e riverificare dal vivo su uno swap reale)
**Priorità:** alta
...
**Aggiornamento 2026-09-24, stesso giorno (Roberto — il fix sopra non è bastato, riprodotto dal vivo):** deployato il fix precedente e testato su uno swap reale flux2→qwen3-14b-q4km. Risultato **misto**:
- Il tracking del PID ha funzionato esattamente come progettato: `_discover_pid_by_window_title` ha trovato e **correttamente scartato** un match su `WindowsTerminal.exe` (log: *"titolo finestra trovato ma il processo è WindowsTerminal.exe... scarto, nessun PID salvato"*); il PID poi effettivamente tracciato/salvato (verificato via `Get-CimInstance`) era il vero `cmd.exe` del backend, non più il contenitore. Nessun `taskkill` emesso da ARIA in tutta la sessione osservata ha preso di mira un PID sbagliato o un titolo diverso da quello esatto del proprio backend.
- **Eppure una finestra PowerShell indipendente (Sniper) si è comunque chiusa**, nello stesso istante in cui Windows ha creato una nuova coppia `WindowsTerminal.exe`+`OpenConsole.exe` per la console di qwen3-14b-q4km (11:48:44) — e quella è rimasta l'unica istanza WindowsTerminal viva sul sistema subito dopo (le istanze precedenti, comprese quelle create per l'avvio di flux2 poco prima, non ci sono più). Diagnosi separata sullo stesso incidente: il **daemon** di Sniper era in realtà già morto per conto proprio 5 minuti prima di qualunque attività di ARIA in quella sessione (causa indipendente, non imputabile ad ARIA — v10.0 del daemon sembra avere un crash proprio pochi secondi dopo l'armamento). Il **viewer** invece non scrive un suo log persistente, quindi non è stato possibile datare il secondo esatto della sua chiusura — ma la sequenza di processi osservata resta compatibile con un comportamento di Windows Terminal stesso (consolidamento/sostituzione di finestre proprie durante la gestione delle console delegate), non con un comando di ARIA: nessun comando eseguito quella sessione era scoping-ato più largo del titolo/PID esatto del proprio backend. `windowingBehavior` in `settings.json` di Windows Terminal risultava non impostato (default Microsoft `useNew`) — comportamento osservato non pienamente coerente con quel default, ma non riproducibile con certezza via sola ispezione SSH.
- **Conclusione**: correggere il tracking del PID (fatto, verificato corretto) non basta finché lo spawn dei backend continua a passare per la delega a Windows Terminal — quella delega è un livello di indirection che Windows gestisce internamente e che ARIA non controlla, indipendentemente da quanto sia preciso il PID tracciato a valle.

**Fix definitivo (2026-09-24, stesso giorno): bypass esplicito della delega a Windows Terminal.** Invece di lasciare che Windows scelga il terminale predefinito per la nuova console (`CREATE_NEW_CONSOLE` da solo non evita la delega — Windows la applica comunque a qualunque nuova console allocata dal sistema), l'Orchestratore invoca ora `conhost.exe` come eseguibile esplicito: `subprocess.Popen(["conhost.exe", "cmd.exe", "/c", f'title {title} && {cmd_str}'], creationflags=subprocess.CREATE_NEW_CONSOLE, ...)`. Richiedere `conhost.exe` esplicitamente è una richiesta di console host classico che Windows onora senza ridelegare a Windows Terminal.

**Verificato dal vivo su PC 139 prima di scrivere il fix nel codice** (test usa e getta, ripulito subito dopo): spawn di prova via `Start-Process -FilePath conhost.exe -ArgumentList @('cmd.exe','/c','title TEST-CONHOST-PROBE && timeout /t 20')` — snapshot dei processi `WindowsTerminal.exe`/`OpenConsole.exe` prima e dopo lo spawn: **zero nuove istanze comparse**. Albero risultante pulito: `conhost.exe → cmd.exe`, nessun intermediario. La finestra resta visibile con i log in tempo reale (esplicitamente richiesto da Roberto: "l'importante è che ogni volta che si avvia un backend... io continui a vedere la finestra con i log") — cambia solo l'aspetto (console classica invece di una scheda Windows Terminal stilizzata), non la funzione.

**Effetto collaterale coerente**: `conhost.exe` diventa ora il PID tracciato in `self._procs[model_id]` (era `cmd.exe` nella versione precedente del fix) — `_discover_pid_by_window_title` non lo rifiuta più come "contenitore generico" (rimosso dalla lista insieme a `WindowsTerminal.exe`/`OpenConsole.exe`): un `conhost.exe` è sempre dedicato a una singola console, mai condiviso tra finestre/app diverse come può esserlo `WindowsTerminal.exe` — è quindi un bersaglio legittimo, non un rischio.

**Ancora da verificare dal vivo**: deploy su PC 139 e un secondo giro di swap reale, con Sniper riarmato, per confermare che stavolta resti vivo.
